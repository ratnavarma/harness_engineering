...
<header><b>ASSEMBLER COCKPIT</b> <span class=meta>local product builder · visible execution trace, not private reasoning</span></header><main><section><h2>Build</h2><textarea id=build-prompt placeholder="Describe the product you want to build…"></textarea><select id=build-mode><option value=yolo selected>Yolo — automatic inside this workdir</option><option value=safe>Safe — requires approval</option><option value=read-only>Read-only</option></select><button class=run id=build-button>Build product</button><p class=meta id=status>Yolo permits local writes automatically. Always-deny commands remain blocked.</p><h2>Sessions</h2><div id=list>Loading…</div></section><section><h2 id=title>Live trace</h2><pre id=tape>Enter a product prompt to start.</pre></section></main>
